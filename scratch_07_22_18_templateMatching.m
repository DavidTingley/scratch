...
%        ls_spikesNREM.times{spk} = Restrict(ls_spikes.times{spk},double(SleepState.ints.NREMstate)); 
    ...
    spkmat_ls = bz_SpktToSpkmat(ls_spikesBEHAV.times,'overlap',5,'binSize',binSize(bins));
    spkmatNREM_ls = bz_SpktToSpkmat(ls_spikesNREM.times,'overlap',5,'binSize',binSize(bins));
...
%         end
    ...
    spkmat_hpc = bz_SpktToSpkmat(hpc_spikesBEHAV.times,'overlap',5,'binSize',binSize(bins));
    spkmatNREM_hpc = bz_SpktToSpkmat(hpc_spikesNREM.times,'overlap',5,'binSize',binSize(bins));
    for t = 1:length(firingMaps.rateMaps)
        template = squeeze(mean(firingMaps.rateMaps{t},2));
        avgTemplateTime = median(diff(behavior.events.trialIntervals(behavior.events.trialConditions==t,:)'));
        ...
        for tc = 1:length(tcFactors)
           ...
           behav_hpc{count_hpc}(tc,:) = corrsAll(tc,spkmatNREM_hpc.timestamps>intervals(1,2) &...
                                spkmatNREM_hpc.timestamps<intervals(3,1));
        end
        figure(2)
        subplot(3,2,3)
