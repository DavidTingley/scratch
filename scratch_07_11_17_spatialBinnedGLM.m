...
% 
positionDecodingGLM_binnedspace_gauss.region = spikes.region;
positionDecodingGLM_binnedspace_gauss.sessionName = spikes.sessionName;
positionDecodingGLM_binnedspace_gauss.UID = spikes.UID;
for cell=1:length(spikes.times)
    positionDecodingGLM_binnedspace_gauss.results{cell} = table;
end
...
%                     circ_smoothTS(phase_trains{cond}{t}(cell,:),smoothing,'method','mean','exclude',0)];
                ...
                rates_trains_smooth = [rates_trains_smooth; ...
                                       Smooth(squeeze(countMap{cond}(cell,t,:)),smoothing)];
                pos = [pos,1:nBins];                   
            end
            ...
            p_cos =cos_phase_trains_smooth;
            p_sin = sin_phase_trains_smooth;
            ...
            positionDecodingGLM_binnedspace_gauss.results{cell} = [positionDecodingGLM_binnedspace_gauss.results{cell}; struct2table(struct)];
...
%                 t_phase = varfun(@mean,positionDecodingGLM_binnedspace_gauss.results{cell},'InputVariables','mse_phase_all',...
%                 'GroupingVariables',{'tau','condition'});
%                 t_both = varfun(@mean,positionDecodingGLM_binnedspace_gauss.results{cell},'InputVariables','mse_both',...
%                 'GroupingVariables',{'tau','condition'});
...
% %                 plot(positionDecodingGLM_binnedspace_gauss.results{cell}.tau(rows),...
% %                     mean(positionDecodingGLM_binnedspace_gauss.results{cell}.mse_rate(rows,:)'),'r')
% %                 hold on
% %                 plot(positionDecodingGLM_binnedspace_gauss.results{cell}.tau(rows),...
% %                     mean(positionDecodingGLM_binnedspace_gauss.results{cell}.mse_phase_all(rows,:)'),'g')
%                 plot(positionDecodingGLM_binnedspace_gauss.results{cell}.tau(rows),...
%                     mean(positionDecodingGLM_binnedspace_gauss.results{cell}.mse_both(rows,:)'),'k')
%                 hold off
% %                 subplot(2,2,3)
...
%             end         
    ...
    positionDecodingGLM_binnedspace_gauss.dateRun = date;  % this can take a very long time so lets save each loop...
save([xml.FileName '.positionDecodingGLM_binnedspace_gauss.cellinfo.mat'],'positionDecodingGLM_binnedspace_gauss')
end
