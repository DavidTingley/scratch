...
for ii=1:length(d)
    ...
    %                 error_phase(count,:) = mean(se_p);
                    ...
                    error_rate_z(count,:) = zscore(mean(se_r));
    %                 error_phase(count,:) = mean(se_p);
                    error_chance_z(count,:) = zscore(mean(se_c));
                    pvals(cellCount,cond) = pval;
                    pvals_rate(cellCount,cond) = pval_r;
                    clear se_*
                    ...
                    figure(region(count))
                    subplot(2,2,1)
                    imagesc(error_rate_z(region==region(count),:))
                    subplot(2,2,2)
    %                 imagesc(error_phase(region==region(count),:))
                    subplot(2,2,3)
                    plot(mean(error_rate_z(region==region(count),:)),'r')
                    hold on
    %                 plot(mean(error_phase(region==region(count),:)),'g')
                    plot(mean(error_chance_z(region==region(count),:)),'k')
                    hold off
                    count=1+count;
                    end
                    end
