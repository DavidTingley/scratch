...
%                 stimTimes{i} = stimTimes_s; clear stimTimes_s
                ...
                stims = sort(stims);
                stimTimes{i} = stims_analog; clear stims_analog
...
for ii=1:length(d)
    if  d(ii).isdir
if ~isempty(rt{ii}) % & ~isempty(SleepState{ii})
    relTime = rt{ii}; 
    abTime = at{ii}; 
    rips = ripples{ii}; 
    sti = stimTimes{ii};
    ...
    responseMag_t = nan(length(stimTimes{ii}),2);
    ...
    parfor s = 1:length(sti)
        ...
        responseMag_t(s,:) = responseMags{ii}(s,:);
    end
...
%     states(i) = nansum(state(ind)==3)./length(ind);
    ...
    if ~isempty(ind)
        responseMags(i,:) = nanmean(responseMagnitude(ind,:));
    end
end
...
save('/home/david/Dropbox/Documents/pubs/inProgress/glucose/data/cgm9.mat','-v7.3')
% whos
% 
% for i=-60:60
