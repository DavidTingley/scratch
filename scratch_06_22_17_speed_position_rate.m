...
for rec = 1:length(d)
    cd(d(rec).name)
% try
    clearvars -except rec d rate_vel rate_acc center_of_mass  rate_vel_pval beh UID spk firingMaps.rateMapss veloc firingMaps.rateMapss fast_trials slow_trials
    c=1;
    xml = LoadParameters;
...
%     [firingMaps.rateMaps countMap opk_rate_vel_corruMap phaseMap] = bz_firingMap1D(spikes.times,behavior,lfp,tau);
    ...
    load([xml.FileName '.placeFields.20_pctThresh.mat'],'fields')
%     %% END LOADING DATA
    ...
    if ~isempty(fields{i}{j}) & length(f) > 20
        ...
        for ii = 1:length(find(behavior.events.trialConditions==i))
            ...
            [peakR(ii) loc] = max(firingMaps.rateMaps{i}(j,ii,fields{i}{j}{ff}.start:fields{i}{j}{ff}.stop));
            loc = loc + fields{i}{j}{ff}.start;
%             [peakR(ii)] = max(firingMaps.rateMaps{i}(j,ii,fields{i}{j}{ff}.COM));
%             if loc > size(firingMaps.rateMaps{1},3)-5
%                 loc = size(firingMaps.rateMaps{1},3)-5;
%             elseif loc < 5
%                 loc = 5;
%             end
%             peakR(ii) = mean(firingMaps.rateMaps{i}(j,ii,loc-4:loc+4)); 
            num_spks{i}{j}(ii) = sum(firingMaps.countMaps{i}(j,ii,:));
...
%       imagesc(squeeze(firingMaps.rateMaps{i}(j,b,:)))
%       subplot(2,2,4)
%       plot(spikes.rawWaveform{j})
        ...
        plot(squeeze(mean(firingMaps.rateMaps{i}(j,slow,:))),'g')
        hold on
        %             plot(squeeze(mean(firingMaps.rateMaps{i}(j,med,:))),'k')
        plot(squeeze(mean(firingMaps.rateMaps{i}(j,fast,:))),'r'); title(j);

        fas_trials(c,:)= squeeze(mean(firingMaps.rateMaps{i}(j,fast,:))); 
        slo_trials(c,:)= squeeze(mean(firingMaps.rateMaps{i}(j,slow,:)));
            pause
        else
...
%             plot(squeeze(mean(firingMaps.rateMaps{i}(j,fast,:))),'r'); title(j);
%             
%             f(c,:)= squeeze(mean(firingMaps.rateMaps{i}(j,fast,:))); 
%             s(c,:)= squeeze(mean(firingMaps.rateMaps{i}(j,slow,:)));
%             
%             subplot(5,2,i)
%             imagesc(squeeze(firingMaps.rateMaps{i}(j,b,:)))
...
%     title(sessionInfo.FileName)
    figure(101)
    subplot(2,2,1)
    scatter(Bin(com,[0 size(firingMaps.rateMaps{1},3)],80),pk_rate_vel_corr,'.')
    ...
    subplot(2,2,2)
    scatter(Bin(com,[0 size(firingMaps.rateMaps{1},3)],80),num_spk_vel_corr,'.')
    ...
    subplot(2,2,3)
    scatter(Bin(com,[0 size(firingMaps.rateMaps{1},3)],80),pk_rate_acc_corr,'.')
    ...
    subplot(2,2,4)
    scatter(Bin(com,[0 size(firingMaps.rateMaps{1},3)],80),num_spk_acc_corr,'.')
    ...
    center_of_mass{rec} = Bin(com,[0 size(firingMaps.rateMaps{1},3)],80);
    rate_vel_pval{rec} = pk_rate_vel_pval;
    beh{rec} = behavior;
    UID{rec}= cellUID;
    spk{rec} = spikes;
    veloc{rec} = velocities;
    rateMaps{rec} = firingMaps.rateMaps;
