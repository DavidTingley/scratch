...
cd(['/ifs/data/buzsakilab/lsDataset/' RECORDING])
...
for i = conditions
    for k=1:length(position{i})
        position{i}{k} = phase_trains{i}{k}(septalCell,:);
    end
end
...
positionDecodingMaxCorr.results = table;r = randperm(length(phase_trains{cond}));
for cond = conditions
for iter = 1:length(r)
...
save(['/ifs/data/buzsakilab/lsDataset/' RECORDING '/' RECORDING '_cell_' num2str(septalCell) '.mat'],'positionDecodingMaxCorr')
end