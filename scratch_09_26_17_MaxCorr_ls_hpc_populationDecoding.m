...
for wind = smoothingRange
        ...
        cl = max_correlation_coefficient_CL;
        cl = train(cl,[phase_trains_smooth_train; theta_train],round(septalResponse_train));
        
        for ts = 1:length(septalResponse_test)
           yfit_circ(ts) = test(cl,[phase_trains_smooth_test(:,ts);theta_test(ts)]); 
        end
        struct.mse_phase = mean((yfit_circ-septalResponse_test).^2);
        
        % cos and sin transformed circular decoding
        cl = max_correlation_coefficient_CL;
        cl = train(cl,[phase_trains_smooth_train_cos; theta_train],round(septalResponse_train));
        
        for ts = 1:length(septalResponse_test)
           yfit_cos(ts) = test(cl,[phase_trains_smooth_test_cos(:,ts);theta_test(ts)]); 
        end
        struct.mse_phase_cos = mean((yfit_cos-septalResponse_test).^2);
        
        cl = max_correlation_coefficient_CL;
        cl = train(cl,[phase_trains_smooth_train_sin; theta_train],round(septalResponse_train));
        
        for ts = 1:length(septalResponse_test)
           yfit_sin(ts) = test(cl,[phase_trains_smooth_test_sin(:,ts);theta_test(ts)]); 
        end
        struct.mse_phase_sin = mean((yfit_sin-septalResponse_test).^2);
        ...
        cl = train(cl,[all_phase_train; theta_train],round(septalResponse_train));
        
        for ts = 1:length(septalResponse_test)
           yfit_circ_all(ts) = test(cl,[all_phase_test(:,ts);theta_test(ts)]); 
        end
        struct.mse_phase_all = mean((yfit_circ_all-septalResponse_test).^2);
        
        % chance phase
        cl = max_correlation_coefficient_CL;
        all_phase_train = [phase_trains_smooth_train_cos;phase_trains_smooth_train_sin];
        all_phase_test =  [phase_trains_smooth_test_cos;phase_trains_smooth_test_sin];
        cl = train(cl,[all_phase_train(:,rr); theta_train],round(septalResponse_train));
        
        for ts = 1:length(septalResponse_test)
           yfit_chance(ts) = test(cl,[all_phase_test(:,rrr(ts));theta_test(ts)]); 
        end
        struct.mse_chance = mean((yfit_chance-septalResponse_test).^2);
        
        %% put data into struct/table
        fits = cell2struct({yfit_chance,yfit_rate,yfit_circ_all,septalResponse_test},{'chance','rate','phase_all','response'},2);
        struct.tau = wind;
...
%             hold off
            ...
            subplot(2,2,2)
            plot(lsDecodingHPC_POP_MaxCorr.results.tau,...
                lsDecodingHPCdbPOP_MaxCorr.results.mse_phase_cos,'g')
            subplot(2,2,3)
            plot(lsDecodingHPC_POP_MaxCorr.results.tau,...
