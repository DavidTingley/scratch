...
for f = 1:length(d)
    ...
    if ~isempty(spikes)
        for i=1:length(spikes.times)
            if strcmp(spikes.region{i},'ca3')% | strcmp(spikes.region{i},'ca3') | strcmp(spikes.region{i},'ca1')
            ISI = diff(spikes.times{i});
            ISIH = hist(diff(spikes.times{i}),0:.001:.2);
            BI(c) = sum(ISIH(1:10))./sum(ISIH(11:end));
            
            
            dd = [true, ISI' < .02, true];  % TRUE if values change
            n = diff(find(dd));               % Number of repetitions
            Y = repelem(n, n);
            
            inBurstSpks = sum(n(find(n>2)));
            nBursts = length(find(n>2));
            BI_Hunt(c) = inBurstSpks ./ length(ISI);
            spksPerBurst(c) = inBurstSpks ./ nBursts;
%             if spksPerBurst(c) > 20
%                 error
%             end
        %     rate(c) = length(spikes.times{i})./lfp.timestamps(end);
            rate(c) = length(spikes.times{i})./spikes.times{i}(end);
            c=1+c;
            end
        end
    end
