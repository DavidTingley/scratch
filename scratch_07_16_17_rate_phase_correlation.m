d  = dir('*201*');

 hpc_corrs =[];
 ls_corrs=[];
  hpc_corrs_shuffle =[];
 ls_corrs_shuffle=[];
 
%% compile data

for ii=1:length(d)
   cd(d(ii).name)
   load([d(ii).name '.behavior.mat'])
    load([d(ii).name '.sessionInfo.mat'])
    spikes = bz_GetSpikes;
    lfp = bz_GetLFP(sessionInfo.thetaChans(2));
    [rateMap countMap occuMap phaseMap] = bz_firingMap1D(spikes.times,behavior,lfp,4);
    rate_phase_corrs.UID = spikes.UID;
    rate_phase_corrs.sessionName = spikes.sessionName;
    rate_phase_corrs.region = spikes.region;
    
   for i=1:length(phaseMap)
    for j=1:length(spikes.times)
        rate_phase_corrs.instRate_phase_corr{j,i}=NaN;
        rate_phase_corrs.instRate_phase_pval{j,i}=NaN;
        rate_phase_corrs.instRateChange_phaseChange_corr{j,i}=NaN;
        rate_phase_corrs.instRateChange_phaseChange_pval{j,i}=NaN;
        rate_phase_corrs.meanRateChangePhaseChangeCorr(j,i)=NaN;
        rate_phase_corrs.meanRatePhaseChangeCorr(j,i)=NaN;
        rate_phase_corrs.instRateChange_phaseChange_corr_shuffle{j,i}=NaN;
        rate_phase_corrs.instRateChange_phaseChange_pval_shuffle{j,i}=NaN;
        rate_phase_corrs.meanRateChangePhaseChangeCorr_shuffle(j,i)=NaN;
        
        if ~isempty(phaseMap{i})
            if size(phaseMap{i}{j},1) > 50  % min 5 spikes across all trials..
                for t = 1:size(rateMap{i},2)
                    f = find(phaseMap{i}{j}(:,2)==t);
                    if length(f) > 5  % min 3 spikes per trial...
     [rate_phase_corrs.instRate_phase_corr{j,i}(t),rate_phase_corrs.instRate_phase_pval{j,i}(t)]= circ_corrcl(...
        [phaseMap{i}{j}(f,end)],phaseMap{i}{j}(f,end-1));
    
     [rate_phase_corrs.instRateChange_phaseChange_corr{j,i}(t),rate_phase_corrs.instRateChange_phaseChange_pval{j,i}(t)]= circ_corrcl(...
        [phaseMap{i}{j}(f,end)],[0; diff(phaseMap{i}{j}(f,end-1))]);
    
    for iter= 1:10
        [rate_phase_corrs.instRateChange_phaseChange_corr_shuffle{j,i}(t,iter),rate_phase_corrs.instRateChange_phaseChange_pval_shuffle{j,i}(t,iter)]= circ_corrcl(...
            [phaseMap{i}{j}(f,end)],circshift([0; diff(phaseMap{i}{j}(f,end-1))]',round(rand*100)));
    end
     rate_phase_corrs.meanRateChangePhaseChangeCorr(j,i) = mean(rate_phase_corrs.instRateChange_phaseChange_corr{j,i});
     rate_phase_corrs.meanRatePhaseChangeCorr(j,i) = mean(rate_phase_corrs.instRate_phase_corr{j,i});
     rate_phase_corrs.meanRateChangePhaseChangeCorr_shuffle(j,i) = mean(rate_phase_corrs.instRateChange_phaseChange_corr_shuffle{j,i}(:));
                    end
                end

            end
        end
    end
   end
   for j=1:length(spikes.times)
       if strcmp(spikes.region{j},'hpc')
   subplot(2,2,1)
   hpc_corrs = [hpc_corrs ,rate_phase_corrs.meanRateChangePhaseChangeCorr(j,:)];
      hpc_corrs_shuffle = [hpc_corrs_shuffle ,rate_phase_corrs.meanRateChangePhaseChangeCorr_shuffle(j,:)];
   histogram(hpc_corrs,0:.05:1,'Normalization','pdf')
   hold on
   histogram(hpc_corrs_shuffle,0:.05:1,'Normalization','pdf','FaceColor','r')
   line([nanmedian(hpc_corrs(:)) nanmedian(hpc_corrs(:))],[0 20],'color','k')
   line([nanmedian(hpc_corrs_shuffle(:)) nanmedian(hpc_corrs_shuffle(:))],[0 20],'color','r')
   hold off
%    set(gca,'yscale','log')
   title('hpc, phase vs rate change corrs')
       elseif strcmp(spikes.region{j},'ls')
   subplot(2,2,2)
   ls_corrs = [ls_corrs , rate_phase_corrs.meanRateChangePhaseChangeCorr(j,:)];
   ls_corrs_shuffle = [ls_corrs_shuffle , rate_phase_corrs.meanRateChangePhaseChangeCorr_shuffle(j,:)];
   histogram(ls_corrs,0:.05:1,'Normalization','pdf')
   hold on
   histogram(ls_corrs_shuffle,0:.05:1,'Normalization','pdf','FaceColor','r')
   line([nanmedian(ls_corrs(:)) nanmedian(ls_corrs(:))],[0 20],'color','k')
   line([nanmedian(ls_corrs_shuffle(:)) nanmedian(ls_corrs_shuffle(:))],[0 20],'color','r')
   hold off
%    set(gca,'yscale','log')
       title('ls')
       end
   end
   pause(.1)
   
   
%    save([rate_phase_corrs.sessionName '.rate_phase_corrs.cellinfo.mat'],'rate_phase_corrs')
   
clear rate_phase*
cd /home/david/datasets/lsDataset/
   
end