...
for rec= 1:length(d)
    ...
    [blah start] = min(abs(hpc.timestamps-min(behavior.events.trialIntervals(:))));
    [blah stop] = min(abs(hpc.timestamps-max(behavior.events.trialIntervals(:))));
    freqs2=25:2:200;
    freqs1=4:0.5:1;

    [cmats_sig_hpc_ls{rec},strength,pref_phase,mean_phase] = thetamod2(double(hpc.data(start:stop)),double(ls.data(start:stop)),freqs2,[2 3 5 6],1250,[25 200]);
    [tort_hpc_ls{rec},freq,MI] = CFCtort(double(hpc.data(start:stop)),double(ls.data(start:stop)),freqs1,freqs2,1250,[30 200]);
    
    [cmats_sig_hpc_hpc{rec},strength,pref_phase,mean_phase] = thetamod2(double(hpc.data(start:stop)),double(hpc.data(start:stop)),freqs2,[2 3 5 6],1250,[25 200]);
    [tort_hpc_hpc{rec},freq,MI] = CFCtort(double(hpc.data(start:stop)),double(hpc.data(start:stop)),freqs1,freqs2,1250,[30 200]);
    
    [cmats_sig_ls_ls{rec},strength,pref_phase,mean_phase] = thetamod2(double(ls.data(start:stop)),double(ls.data(start:stop)),freqs2,[2 3 5 6],1250,[25 200]);
