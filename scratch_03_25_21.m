...
for i=1:length(d)
   ...
   sessionInfo = bz_getSessionInfo;
    ...
    if exist([d(i).name '.CA1Ripples.events.mat']) & exist([d(i).name '.spikes.cellinfo.mat']) % if we have spikes and ripples for this rec
       ...
       spikes = bz_GetSpikes('region','acg');
       if isempty(spikes)
           spikes = bz_GetSpikes('region','ppc'); % I was lazy and labelled diff recordings CA1 or HPC.. and still haven't changed things
       end
       ...
       if length(spikes.times) >= 5 % threshold, need at least 5 cells to use recording
           ...
           populationZscoredRate = zscore(fastrms(rate,20));
