...
%     
    ...
    if ~exist([sessionInfo.FileName '.SleepState.states.mat'])
        SleepScoreMaster(pwd)
    end
%     read_Intan_RHD2000_file
%     num_channels = length(board_adc_channels);  fileinfo = dir('analogin.dat'); num_samples = fileinfo.bytes/(num_channels * 2); fid = fopen('analogin.dat', 'r');
...
%             c=c+1;
%             end
%         end sessionInfo = bz_getSessionInfo(pwd,'noPrompts',true);
%         clear pul;
%         pul{1} = stims;
%         save([folders(f).name '.stimTimes.mat'],'pul')
%     end
...
%         noise = lfp.channels(noi);
        lfp = bz_GetLFP(ch);
%         noise = bz_GetLFP(noi);
        [ripples] = bz_FindRipples(lfp.data,lfp.timestamps,'emgThresh',0.85,'durations',[20 200]);
        [b a]=butter(4,[140/625 180/625],'bandpass');
...
%         read_Intan_RHD2000_file
        num_channels = 1;%length(board_adc_channels);  
    ...
    %     v = single(v);
        [pul, val] = getPulses_thresh(v,'fs',20000);
        sessionInfo = bz_getSessionInfo;
        save([folders(f).name '.stimTimes.mat'],'pul','val')
    end
    cd ..
end
% 
