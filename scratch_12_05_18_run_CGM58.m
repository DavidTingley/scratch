...
cd /mnt/nyuShare/Homes/dwt244/glucoseRev/LS_dreadd/CGM58
...
for i=1:length(d)
    if d(i).isdir
        ...
        sharpWaves{i} = bz_LoadEvents(pwd,'sharpWaves');
...
%% daylight savings adjustment here???
% burn the first hour to align with ts and recordings
isig_levels(1:end-11) = isig_levels(12:end);
isig_levels(end-12:end)=[nan];
%%
d = dir(['*' animal '*']);


remove = [1:10 1173:1189];
% 1860:2419
...
for ii= [1:length(d)]
    if  d(ii).isdir
        if ~isempty(rt{ii}) & ~isempty(sharpWaves{ii})
        for i=1:length(sharpWaves{ii}.timestamps)
            [a b]= min(abs(rt{ii}-sharpWaves{ii}.timestamps(i)));
            [aa bb] = min(abs(at{ii}(b)-absTime));

            absolute_sw(vsc) = absTime(bb);
...
%     ind = find(InIntervals(absolute,[absTime(i)-1.15741277113557e-05 absTime(i)]));
    ...
    if exist('absolute_sw')
       ind = find(InIntervals(absolute_sw,[absTime(i)-1.15741277113557e-05*60*2.5 absTime(i)+1.15741277113557e-05*60*2.5]));
       count_sw(i) = length(ind); 
    end
...
% count(isnan(rec))=nan;
% count_sw(isnan(rec))=nan;




% save('C:\Users\SB13FLLT001\Dropbox\Documents\pubs\inProgress\glucose\data\revision\bruce.mat','-v7.3')
save('/home/david/Dropbox/Documents/pubs/inProgress/glucose/data/revision/CGM58.mat','-v7.3')
