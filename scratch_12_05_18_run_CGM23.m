...
cd(['/mnt/packrat/userdirs/david/zpool3/' animal])
...
%                 stimTimes{i} = stimTimes_s; clear stimTimes_s
                responses{i} = [];
                stimTime{i} = [];
            elseif exist([d(i).name '.responseMagnitudes.mat'])
...
%                 end
            ...
            if ~exist([d(i).name '.specSlope.mat'])
                ...
                lfp = bz_GetLFP(sessionInfo.left);
...
%                 lfp.samplingRate = 1250;
            ...
            if ~exist([d(i).name '.thetaPower.mat'])
                ...
                lfp = bz_GetLFP(sessionInfo.left);
                temp = decimate(filtfilt(bb,aa,double(lfp.data)),1250);
...
for ii=1:length(d)
    if  d(ii).isdir
if ~isempty(rt{ii}) & ~isempty(stimTimes{ii}) %& ~isempty(ripples{ii}) % & ~isempty(SleepState{ii})
    relTime = rt{ii}; 
    abTime = at{ii}; 
    rips = ripples{ii}; 
    sti = stimTimes{ii};
...
%             id = round(sti(s)*1250);
%             if id > 625 & id+625<length(rips.detectorinfo.detectionparms.lfp)
%                 responseMag_t(s,:) = fastrms(filtfilt(hi,lo,double(rips.detectorinfo.detectionparms.lfp(id-625:id+625))),200);
%                 stimTime_t(s) = sti(s);
% %                 c_stim = 1 + c_stim; 
%             end
%         end
...
%     states(i) = nansum(state(ind)==3)./length(ind);
    ...
    responseMags(i,:) = nanmedian(responseMag(ind,:));
...
save('/home/david/Dropbox/Documents/pubs/inProgress/glucose/data/stim/cgm23.mat','-v7.3')
% whos
% 
% for i=-60:60
