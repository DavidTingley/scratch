...
%             end
            ...
            if ~exist([d(i).name '.thetaResid.mat'])
                ...
                thetaPow_resid{i}(end+1) = thetaPow_resid{i}(end);
            else
...
%                 thetaPow_raw{i} = thetaPow_raw_s; clear thetaPow_raw_s
                thetaPow_z{i} = thetaPow_z_s; clear thetaPow_z_s
                thetaPow_z{i}(end+1) = thetaPow_z{i}(end);
            end
        else
            emgFromLFP{i} = [];
            thetaPow_z{i} = [];
...
remove = [1:50, 1234:1360, 2325:2520, 3020:3105, 3264:3275];
% 1860:2419
...
% clear ripples
% save('/home/david/Dropbox/Documents/pubs/inProgress/glucose/data/CGM1.mat','MUA*','theta','count','idx','isig_levels','ripples','states','amp*','dur*','freq*','rip*','-v7.3')
save('C:\Users\SB13FLLT001\Dropbox\Documents\pubs\inProgress\glucose\data\_CGM1.mat','-v7.3')
% whos
% 
% 
% 
...
% for i=-60:60
% [beta(i+61,:) dev(i+61) stats(i+61)]=glmfit([theta',count',amps',dur',freq',spSlope',states',zt'],circshift([0,diff(isig_levels)]',-i),'normal');
% end
% for i=1:121
% p(i,:) = stats(i).p;
% end
