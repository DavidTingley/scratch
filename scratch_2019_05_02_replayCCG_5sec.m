...
fieldCounts = [];
for w=1:220
%     compression(w) = 4801/(length(100+10*(w-1):4900-10*(w-1)));
    compression(w) = length((221-w:281+w))./ length((1+w:501-w));
end
...
%     load([sessionInfo.FileName '.CA1Ripples.events.mat'])
    ...
    [ccg_beh_jitter t] = CCG(times_jitter,groups,'binSize',.001,'duration',.5);
...
%     spk.times{i} = [Restrict(spikes.times{i},[ripples.timestamps(idx(1),1)-.05 spikes.spindices(end,1)])];
    ...
    [times groups] = spikes2sorted(spk.times); clear spk
    if ~isempty(times) & sum(sum(~sh(ls_idx,ls_idx))) > 0
        [ccg_ls_rip t] = CCG(times,groups,'binSize',.001,'duration',.5);
        ccg_ls_rip(:,length(spikes.times),length(spikes.times))=0;
        
        times_jitter = times + rand(length(times),1)./5;
        [ccg_ls_rip_jitter t] = CCG(times_jitter,groups,'binSize',.001,'duration',.5);
...
%         temporalBias_post{ii} = nan(size(ccg_ls_rip,2),size(ccg_ls_rip,3));
        warpedCorr_post{ii} = nan(size(ccg_ls_rip,2),size(ccg_ls_rip,3),220);
        warpedCorr_post_shuf{ii} = nan(size(ccg_ls_rip,2),size(ccg_ls_rip,3),1,220);
...
%                 if sum(ccg_beh(:,i,j)) > 5
                    ...
                    [pk1 loc1] = max(Smooth(c1,15));
                    [pk2 loc2] = max(Smooth(c2,15));
                    postPeaks{ii}(i,j,:) = [loc1 loc2];
                    
                    for w=1:220
%                        compression(w) = 4801/(length(100+10*(w-1):4900-10*(w-1)));
                       warpedCorr_post{ii}(i,j,w) = single(corr(makeLength(c1(221-w:281+w),121)',makeLength(c2(1+w:501-w),121)'));
%                         warpedCorr_post{ii}(i,j,w) = single(corr(makelength(c1(2381-w*10:2621+w*10),121)',c2(2441:2561)));
                       for iter = 1
%                        warpedCorr_post_shuf{ii}(i,j,iter,w) = single(corr((c1(100:4900)),circshift(makeLength((c2(100+10*(w-1):4900-10*(w-1))),4801)',randi(240))));
                       warpedCorr_post_shuf{ii}(i,j,iter,w) = single(corr(makeLength(c1(221-w:281+w),121)',circshift(makeLength(c2(1+w:501-w),121)',randi(121))));
                       end
                    end
%                 end
                ...
                nSpikes_post{ii}(i,j) = sum(ccg_ls_rip(:,i,j));
                    ...
                    warpedCorr_post{ii}(i,j,1:240) = single(NaN);
                    warpedCorr_post_shuf{ii}(i,j,1,1:240) = single(NaN);
...
%         temporalBias_pre{ii} = nan(size(ccg_ls_rip,2),size(ccg_ls_rip,3));
        warpedCorr_pre{ii} = nan(size(ccg_ls_rip,2),size(ccg_ls_rip,3),220);
        warpedCorr_pre_shuf{ii} = nan(size(ccg_ls_rip,2),size(ccg_ls_rip,3),1,220);
        for i = 1:size(ccg_ls_rip,2)
            for j = 1:size(ccg_ls_rip,3)
                if sum(ccg_ls_rip(:,i,j)) > 5 & sh(i,j) == 0 & ls_idx(i) == 1 & ls_idx(j) == 1 & sum(ccg_beh(:,i,j)) > 5
...
%                 if sum(ccg_beh(:,i,j)) > 5
                    ...
                    [pk1 loc1] = max(Smooth(c1,15));
                    [pk2 loc2] = max(Smooth(c2,15));
                    prePeaks{ii}(i,j,:) = [loc1 loc2];
                    
                    
                    for w=1:220
%                        compression(w) = 4801/(length(100+10*(w-1):4900-10*(w-1)));
%                        warpedCorr_pre{ii}(i,j,w) = single(corr((c1(100:4900)),makeLength((c2(100+10*(w-1):4900-10*(w-1))),4801)'));
                        warpedCorr_pre{ii}(i,j,w) =  single(corr(makeLength(c1(221-w:281+w),121)',makeLength(c2(1+w:501-w),121)'));
                       for iter = 1
%                        warpedCorr_pre_shuf{ii}(i,j,iter,w) = single(corr((c1(100:4900)),circshift(makeLength((c2(100+10*(w-1):4900-10*(w-1))),4801)',randi(240))));
                        warpedCorr_pre_shuf{ii}(i,j,iter,w) = single(corr(makeLength(c1(221-w:281+w),121)',circshift(makeLength(c2(1+w:501-w),121)',randi(121))));
                       end
                    end
%                 end
                ...
                nSpikes_pre{ii}(i,j) = sum(ccg_ls_rip(:,i,j));
                    ...
                    warpedCorr_pre{ii}(i,j,1:240) = single(NaN);
                    warpedCorr_pre_shuf{ii}(i,j,1,1:240) = single(NaN);
...
post_excess_jitter_corr(idx)=nan;
pre_excess_jitter_corr(idx)=nan;
beh_excess_jitter_corr(idx)=nan;


save('/home/david/Dropbox/HPC_replay_dataset_unsmoothed_5spks_allThresh_20190831.mat','-v7.3')
