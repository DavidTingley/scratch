...
for i=1:length(d)
    if d(i).isdir
        cd(d(i).name);
        if exist([d(i).name '.sessionInfo.mat'])
...
%         vRipples{i} = bz_LoadEvents(pwd,[num2str(threshold,'%06.f') '.VentralRipples']);
        ...
        vsharpWaves{i} = bz_LoadEvents(pwd,'VentralSharpWaves');
...
remove = [1:72 632:637];
% 1860:2419
isig_levels = circshift(isig_levels,-7);
...
idx = 50:500;
save(['/media/data/Dropbox/Documents/pubs/inProgress/glucose/data/revision/Vanessa.mat'],'-v7.3')

% save(['/home/david/Dropbox/Documents/pubs/inProgress/glucose/data/revision/dvSweep/' num2str(threshold,'%06.f') '_Vanessa.mat'],'-v7.3')
%     end
% end
