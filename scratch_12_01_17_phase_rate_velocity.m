...
for rec = 1:length(d)
    cd(d(rec).name)
    if ~isempty(dir('*positionDecodingMaxCorr_binned_box_mean*')) & exist([d(rec).name '.placeFields.20_pctThresh.mat'])
    load([d(rec).name '.positionDecodingMaxCorr_binned_box_mean.cellinfo.mat'])
...
%% iterate through conditions
    ...
    if ~strcmp(phaseMaps.region{cell},'ls') & ~isempty(positionDecodingMaxCorr_binned_box_mean.results{cell})
...
%             end
        ...
        phaseCorr_delta(count) = circ_corrcl(diff(dat{count}(:,3)),dat{count}(1:end-1,1));
        rateCorr_delta(count) = corr(diff(dat{count}(:,2)),dat{count}(1:end-1,1));
        ...
        for iter = 1:100
           ...
           phaseCorr_delta_shuffle(count,iter) = circ_corrcl(diff(dat{count}(r,3)),dat{count}(1:end-1,1));
           rateCorr_delta_shuffle(count,iter) = corr(diff(dat{count}(r,2)),dat{count}(1:end-1,1));
        end
        else
            phaseCorr(count) = nan;
            rateCorr(count) = nan;
