...
[bbb aaa] = butter(3,[40/625 150/625],'bandpass');
for ii=1:length(d)
    cd(d(ii).name)
    sessionInfo = bz_getSessionInfo;
    disp(['working on: ' sessionInfo.FileName])
    if exist([sessionInfo.FileName '.lfp.mat'])
...
%     end
    ...
    if ~isempty(sessionInfo.ca1)
        for i=1:length(behavior.events.trialConditions)
            ...
            if stop-start > 1000
            [coherogram phase t f] = bz_MTCoherogram(double(ls.data(start:stop,1)),double(ca1.data(start:stop,1)),'range',[4 12],'window',.3,'step',.15);
            coh_ca1(c1,:) = makeLength(mean(coherogram),1000);
            [coherogram phase t f] = bz_MTCoherogram(angle(hilbert(FiltFiltM(b,a,double(ls.data(start:stop,1))))),angle(hilbert(FiltFiltM(b,a,double(ca1.data(start:stop,1))))),'range',[4 12],'window',.3,'step',.15);
            coh_ca1_phase(c1,:) = makeLength(mean(coherogram),1000);
            coh_ca1_z(c1,:) = zscore(coh_ca1(c1,:));
            coh_ca1_phase_z(c1,:) = zscore(coh_ca1_phase(c1,:));
            
            [coherogram phase t f] = bz_MTCoherogram(fastrms(FiltFiltM(bbb,aaa,double(ls.data(start:stop,1))),60),angle(hilbert(FiltFiltM(b,a,double(ca1.data(start:stop,1))))),'range',[4 12],'window',.3,'step',.15);
    ...
    if ~isempty(sessionInfo.ca3)
        for i=1:length(behavior.events.trialConditions)
            ...
            [coherogram phase t f] = bz_MTCoherogram(fastrms(FiltFiltM(bbb,aaa,double(ls.data(start:stop,1))),60),angle(hilbert(FiltFiltM(b,a,double(ca3.data(start:stop,1))))),'range',[4 12],'window',.3,'step',.15);
