...
%         SleepScoreMaster(pwd)
        ch = 4; %input('lfp chan: ');
        noi = 0; %input('noise chan: ');
%         chan = lfp.channels(ch);
%         noise = lfp.channels(noi);
        lfp = bz_GetLFP(ch);
%         noise = bz_GetLFP(noi);
        [ripples] = bz_FindRipples(lfp.data,lfp.timestamps,'stdev',30000,'emgThresh',0,'durations',[20 200]);
        [b a]=butter(4,[140/625 180/625],'bandpass');
